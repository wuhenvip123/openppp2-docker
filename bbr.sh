...
# TCP 配置
net.ipv4.tcp_syncookies = 0  # 禁用syncookies 洪水攻击保护
...
* soft nofile $((1024 * 1024))  # 为所有用户设置软文件描述符限制
* hard nofile $((1024 * 1024))  # 为所有用户设置硬文件描述符限制
* soft nproc unlimited  # 为所有用户设置软进程数限制
* hard nproc unlimited  # 为所有用户设置硬进程数限制
* soft core unlimited  # 为所有用户设置软 core 文件大小限制
* hard core unlimited  # 为所有用户设置硬 core 文件大小限制
EOF
